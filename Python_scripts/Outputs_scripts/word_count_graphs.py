...
axs[0].scatter(word_count,unique_word_count, color = '#5dde5b', s = 100)
#label each point (Kanye West needs different positioning so not to cause overlap)
for i, txt in enumerate(word_count_df[0]):
    if txt == 'Kanye West':
        ax.annotate(txt, (word_count[i] -100,unique_word_count[i]+0.8), fontweight = 'bold')
    else:
...
axs[1].set_xticklabels(word_count_df[0],rotation = 'vertical')
plt.show()